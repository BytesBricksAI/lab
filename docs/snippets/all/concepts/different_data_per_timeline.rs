...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rec.log(
        "points",
        &simplant_lab::Points2D::new([(0.0, 0.0), (1.0, 1.0)]),
    )?;
    ...
    rec.log(
        "points",
        &simplant_lab::Points2D::update_fields().with_colors([0xFF0000FF]),
    )?;
    ...
    rec.log(
        "points",
        &simplant_lab::Points2D::update_fields().with_colors([0x0000FFFF]),
    )?;
