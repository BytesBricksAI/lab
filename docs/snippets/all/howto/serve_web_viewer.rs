...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rec = simplant_lab::RecordingStreamBuilder::new(
        "rerun_example_serve_web_viewer",
    )
    .serve_grpc()?;
    ...
    let _server_guard = simplant_lab::serve_web_viewer(
        simplant_lab::web_viewer::WebViewerConfig {
    ...
    rec.log(
        "data",
        &simplant_lab::Boxes3D::from_half_sizes([(2.0, 2.0, 1.0)]),
    )?;
