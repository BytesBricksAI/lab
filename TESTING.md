...
Redap stands for "Rerun data protocol." It is the interface between clients such as the SimPlant Lab viewer or SDK, and servers such as SimPlant Lab OSS or Rerun Hub.

We have several test harnesses related to redap.

### `re_redap_tests`

This is a Rust-based compliance test suite that builds directly against the server's service handler. It is run both against the OSS server in this repository, and Rerun's proprietary implementation Rerun Hub. This test suite does not run through an actual gRPC connection. It directly links to the servers' code.
