...
You can try running the Viewer in a browser using `spl.serve()` in Python, or using `simplant-lab --web-viewer mydata.rrd`.
...
NOTE: `.rrd` files do not yet offer full backwards or forwards compatibility. However, the current version of SimPlant Lab will always be able to open `.rrd` files generated by the *previous* version. This is not guaranteed to work for files generated with older versions.
...
The SimPlant Lab Viewer uses the [`wgpu`](https://github.com/gfx-rs/wgpu) graphics API. It provides a high-performance abstraction over Vulkan, Metal, D3D12, D3D11, OpenGLES, WebGL and [WebGPU](https://en.wikipedia.org/wiki/WebGPU). This lets us write the same code graphics code for native as for web.
...
The GUI in the SimPlant Lab Viewer is using [`egui`](https://www.egui.rs/), a cross-platform, [immediate mode GUI](https://github.com/emilk/egui#why-immediate-mode).
...
The SimPlant Lab Viewer can be compiled to Wasm and run in a browser.
...
The SimPlant Lab Viewer uses an [immediate mode GUI](https://github.com/emilk/egui#why-immediate-mode), [`egui`](https://www.egui.rs/). This means that each frame the entire GUI is being laid out from scratch.

In fact, the whole of the SimPlant Lab Viewer is written in an immediate mode style. Each rendered frame it will query the in-RAM data store, massage the results, and feed it to the renderer.
...
| Crate            | Description                               |
| ---------------- | ----------------------------------------- |
| simplant-lab-cli | SimPlant Lab native CLI binary crate      |
| simplant-lab     | SimPlant Lab Rust SDK and Viewer shim crate |
| rerun_c          | SimPlant Lab C SDK                        |
| rerun_py         | SimPlant Lab Python SDK                   |
| re_sdk           | SimPlant Lab logging SDK                  |
...
| re_component_fallbacks | Provides default fallbacks for SimPlant Lab components in the fallback provider registry.                         |
| re_component_ui        | Provides UI editors for SimPlant Lab component data for registration with the SimPlant Lab Viewer component UI registry. |
...
| re_time_panel          | The time panel of the SimPlant Lab Viewer, allowing to control the displayed timeline & time.                    |
...
| re_view_time_series    | A View that shows plots over SimPlant Lab timelines.                                                             |
| re_viewer              | The SimPlant Lab Viewer                                                                                          |
| re_viewport            | The central viewport panel of the SimPlant Lab viewer.                                                           |
...
| re_data_ui            | Provides UI elements for SimPlant Lab component data for the SimPlant Lab Viewer. |
| re_renderer           | A wgpu-based renderer for all your visualization needs.                  |
| re_renderer_examples  | Examples for re_renderer                                                   |
| re_ui                 | SimPlant Lab GUI theme and helpers, built around egui                    |
| re_viewer_context     | SimPlant Lab Viewer state that is shared with the viewer's code components. |
...
| Crate           | Description                                                         |
| --------------- | ------------------------------------------------------------------- |
| re_dataframe    | The SimPlant Lab public data APIs.                                  |
| re_datafusion   | DataFusion interfaces to SimPlant Lab gRPC queries                    |
| re_entity_db    | In-memory storage of SimPlant Lab entities                          |
| re_log_encoding | Helpers for encoding and transporting SimPlant Lab log messages     |
| re_protos       | SimPlant Lab remote store gRPC API types                            |
...
| re_sdk_types    | The built-in SimPlant Lab data types, component types, and archetypes. |

### Low-level store

| Crate          | Description                                                                                         |
| -------------- | --------------------------------------------------------------------------------------------------- |
| re_chunk       | A chunk of SimPlant Lab data, encoded using Arrow. Used for logging, transport, storage and compute. |
| re_chunk_store | An in-memory time series database for SimPlant Lab log data, based on Apache Arrow.                 |
| re_log_types   | The basic building blocks of the SimPlant Lab data types and tables.                                |
| re_sorbet      | SimPlant Lab arrow metadata and record batch definitions.                                           |
| re_types_core  | The core traits and types that power SimPlant Lab's data model.                                     |

### Data flow

| Crate                | Description                                                          |
| -------------------- | -------------------------------------------------------------------- |
| re_redap_client      | Official client for the SimPlant Lab Data Protocol                   |
| re_redap_tests       | Official test suite for the SimPlant Lab Data Protocol               |
| re_importer          | Handles importing of SimPlant Lab data from file using importer plugins |
| re_parquet           | Parquet file loading with configurable column grouping               |
| re_data_source       | Handles loading of SimPlant Lab data from different sources          |
| re_grpc_client       | Client for the legacy StoreHub API                                   |
| re_grpc_server       | Server for the legacy StoreHub API                                   |
| re_web_viewer_server | Serves the SimPlant Lab web viewer (Wasm and HTML) over HTTP         |
...
| re_dev_tools      | Various tools for SimPlant Lab development. Each tool has a subcommand. |
| re_protos_builder | Generates code for SimPlant Lab remote store gRPC API                  |
| re_types_builder  | Generates code for SimPlant Lab's SDKs from flatbuffers definitions.   |

### Utilities

| Crate                | Description                                                                               |
| -------------------- | ----------------------------------------------------------------------------------------- |
| re_analytics         | SimPlant Lab's analytics SDK                                                              |
...
| re_case              | Case conversions, the way SimPlant Lab likes them                                         |
...
| re_lenses            | A collection of lenses for mapping component data to SimPlant Lab semantic types.         |
| re_lenses_core       | Core lens types and composable Arrow array transformations                                |
| re_log               | Helpers for setting up and doing text logging in the SimPlant Lab crates.                 |
| re_log_channel       | An in-memory channel of SimPlant Lab data messages                                        |
| re_mcap              | Convert MCAP into SimPlant Lab-compatible data.                                           |
...
| re_perf_telemetry    | In and out of process performance profiling utilities for SimPlant Lab & Redap            |
...
| re_uri               | Parsing and constructing of SimPlant Lab URIs                                             |
