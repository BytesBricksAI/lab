...
use crate::commands::AcquireCommand;
#[cfg(feature = "analytics")]
use crate::commands::AnalyticsCommands;
use crate::commands::AssetsCommand;
use crate::commands::DatasetCommand;
...
The SimPlant-Lab command-line interface:
...
Examples:
    Open a SimPlant-Lab that listens for incoming SDK connections:
        simplant-lab

    Load some files and show them in the SimPlant-Lab:
        simplant-lab recording.rrd mesh.obj image.png https://example.com/recording.rrd

    Open an .rrd file and stream it to a Web Viewer:
        simplant-lab recording.rrd --web-viewer
    ...
    Host a Rerun gRPC server which listens for incoming connections from the logging SDK, buffer the log messages, and serve the results:
        simplant-lab --serve-web

    Host a SimPlant-Lab server which serves a recording from a file over gRPC to any connecting SimPlant-Labs:
        simplant-lab --serve-web recording.rrd

    Host a Rerun gRPC server without spawning a Viewer:
        simplant-lab --serve-grpc

    Spawn a Viewer without also hosting a gRPC server:
        simplant-lab --connect

    Connect to a SimPlant-Lab server:
        simplant-lab rerun+http://localhost:9877/proxy

    Listen for incoming gRPC connections from the logging SDK and stream the results to disk:
        simplant-lab --save new_recording.rrd
"#;
...
struct Args {
    ...
    #[clap(
        long,
        long_help = r"An upper limit on how much memory the SimPlant-Lab should use.
When this limit is reached, SimPlant-Lab will drop the oldest data.
...
Example: `16GB` or `50%` (of system total)."
    ...
    #[clap(
        ...
        long_help = r"Whether the SimPlant-Lab should persist the state of the viewer to disk.
When persisted, the state will be stored at the following locations:
- Linux: `/home/UserName/.local/share/simplant-lab`
- macOS: `/Users/UserName/Library/Application Support/SimPlant-Lab`
- Windows: `C:\Users\UserName\AppData\Roaming\rerun`"
    ...
    //
    // TODO(andreas): The Rust/Python APIs deprecated `serve_web` and instead encourage separate usage of `rec.serve_grpc()` + `simplant_lab::serve_web_viewer()` instead.
...
If no arguments are given, a server will be hosted which a SimPlant-Lab SDK can connect to.")]
    ...
    ///
    /// Requires SimPlant-Lab to have been compiled with the `web_viewer` feature.
    ///
    ...
    /// Hide the normal SimPlant-Lab welcome screen.
    #[clap(long)]
...
impl Args {
    fn generate_markdown_manual() -> String {
        ...
        fn generate_markdown_manual(
            ...
            // * `reset`: Reset the memory of the SimPlant-Lab
            // """
...
enum Command {
    ...
    /// Reset the memory of the SimPlant-Lab.
    ///
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    #[expect(unsafe_code)]
    unsafe {
        std::env::set_var("OTEL_SERVICE_NAME", "simplant-lab");
    }
    ...
    let res = if let Some(command) = args.command {
        match command {
            ...
            Command::Assets(cmd) => cmd.run(),

            Command::Acquire(cmd) => cmd.run(),

            Command::Dataset(cmd) => cmd.run(),
        ...
        Box::new(move |cc| {
            {
                ...
                tokio::spawn(async move {
                    ...
                    match tokio::signal::ctrl_c().await {
                        Ok(()) => {
                            re_log::info!("Caught Ctrl-C, quitting SimPlant-Lab…");
...
fn is_another_server_already_running(server_addr: std::net::SocketAddr) -> bool {
    ...
    if TcpStream::connect_timeout(&server_addr, std::time::Duration::from_secs(1)).is_ok() {
        re_log::info!(
            %server_addr,
            "A process is already listening at this address. Assuming it's a SimPlant-Lab."
        );
        true
    } else {
        false
...
fn record_cli_command_analytics(args: &Args) {
    ...
    let (command, subcommand) = match command {
        ...
        Some(Command::Assets(_)) => ("assets", None),

        Some(Command::Acquire(_)) => ("acquire", None),

        Some(Command::Dataset(_)) => ("dataset", None),
