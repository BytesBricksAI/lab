...
//! # fn positions() -> Vec<simplant_lab::Position3D> { Default::default() }
//! # fn colors() -> Vec<simplant_lab::Color> { Default::default() }
//! // Stream log data to an awaiting `rerun` process.
//! let rec = simplant_lab::RecordingStreamBuilder::new("rerun_example_app").connect_grpc()?;
//!
//! let points: Vec<simplant_lab::Position3D> = positions();
//! let colors: Vec<simplant_lab::Color> = colors();
//! let image: image::DynamicImage = capture_image();
//!
//! rec.set_time_sequence("frame", 42);
//! rec.log("path/to/points", &simplant_lab::Points3D::new(points).with_colors(colors))?;
//! rec.log("path/to/image", &simplant_lab::Image::from_color_model_and_tensor(simplant_lab::ColorModel::RGB, image)?)?;
//!
...
//!
//! ```no_run
//! let rec = simplant_lab::RecordingStreamBuilder::new("rerun_example_app").save("my_data.rrd")?;
//!
//! # Ok::<(), Box<dyn std::error::Error>>(())
//! ```
//!
...
//! # fn log_to(rec: &simplant_lab::RecordingStream) {}
//! let (rec, storage) = simplant_lab::RecordingStreamBuilder::new("rerun_example_app").memory()?;
//! log_to(&rec);
//!
//! // Will block program execution!
//! simplant_lab::native_viewer::show(main_thread_token, storage.take());
//!
//! # Ok::<(), Box<dyn std::error::Error>>(())
//! ```
//!
